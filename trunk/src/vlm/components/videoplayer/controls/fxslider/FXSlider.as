...
public class FXSlider extends UIComponent
{
	...
	protected function get boundMin():Number
	{
		return thumb.width/2
	}
	
	protected function get boundMax():Number
	{
	...
	override protected function createChildren():void
	{
		...
		if(!getStyle("trackColor"))
			setStyle("trackColor", VLMParams.gray);
			
		if(!getStyle("thumbColor"))
			setStyle("thumbColor", VLMParams.red);
			
		if(!getStyle("thumbOutlineColor"))
			setStyle("thumbOutlineColor", VLMParams.red);
	...
	override protected function updateDisplayList(unscaledWidth:Number, unscaledHeight:Number):void
    {
		...
		if(valueChanged)
        {
        	thumb.x = Math.round((_value/_maximum)*(unscaledWidth ));
        	valueChanged = false;
        }
        ...
        g.clear();
        g.beginFill(VLMParams.lightBlue, 0);
	...
	private function onMouseMove(event:MouseEvent):void
	{
		...
		trace("boundMin: " + boundMin );
		trace("boundMax: " + boundMax );
